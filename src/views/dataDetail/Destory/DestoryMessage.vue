      ...
      <div class="equipment-message">
        <div class="equipment-header">
          <div class="equipment-header-left">
            <div class="drawer-form-title">销毁资料信息</div>
          </div>
        </div>
        <leadal-table
...
import { loadRecycle, pageMediaRecycle } from "@/api/data";
export default {
  name: "DestoryMessage",
  inject: ["root"],
  components: {
  ...
  methods: {
    async getData() {
      ...
      const res = await loadRecycle(params);
    ...
    async loadDetail() {
      ...
      const res = await pageMediaRecycle(params);
