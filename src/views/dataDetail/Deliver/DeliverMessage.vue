      ...
      <div class="equipment-message">
        <div class="equipment-header">
          <div class="equipment-header-left">
            <div class="drawer-form-title">外送资料信息</div>
          </div>
        </div>
        <leadal-table
...
import { loadTakeout, pageTakeout2Media } from "@/api/data";
export default {
  ...
  methods: {
    async getData() {
      ...
      const res = await loadTakeout(params);
    ...
    async loadDetail() {
      const params = {
        currentPage: 1, //当前页
        pageSize: 10, //单前每页几条
        takeoutId: this.root.id, //记录主键
      };
      const res = await pageTakeout2Media(params);
