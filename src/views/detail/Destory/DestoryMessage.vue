  ...
  <div>
    <div class="drawer-form">
      ...
      >
        ...
        <el-form-item>
          <label slot="label" style="font-weight: 400">销毁事由</label>
      ...
      <div class="equipment-message">
        <div class="equipment-header">
          <div class="equipment-header-left">
            <div class="drawer-form-title">销毁设备信息</div>
          </div>
        </div>
        <leadal-table
...
import { viewDetailDestory } from "@/api/equipment";
export default {
  ...
  methods: {
    async getData() {
      ...
      const params = {
        id: this.root.id,
        currentUserId: this.root.toUserId,
      };
      const res = await viewDetailDestory(params);
      const editObj = res.data;
      Object.assign(this.applyForm, editObj.destoryData);
      this.$emit("update:applyForm", this.applyForm);
      this.tableObj.tableData = editObj.devInfo.data;
      this.loading = false;
    },
  },
};
